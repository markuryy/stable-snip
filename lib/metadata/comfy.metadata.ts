...
// Clean bad JSON output from ComfyUI
...
// AIR keys for Civitai resource format
...
export const comfyMetadataProcessor = {
  ...
  parse: (exif: Record<string, any>) => {
    ...
    if (exif.extraMetadata && typeof exif.extraMetadata === 'object' && exif.extraMetadata.prompt) {
      ...
      if (initialSamplerNode) {
        ...
        if (initialSamplerNode.latent_image?.inputs) {
  ...
  encode: (meta: Record<string, any>) => {
    const comfy = typeof meta.comfy === 'string' 
      ? JSON.parse(meta.comfy)
      : meta.comfy;
